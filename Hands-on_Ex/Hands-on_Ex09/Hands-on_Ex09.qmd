...
- [cor.vars]{style="color:purple;"} argument is used to compute the correlation matrix needed to build the corrgram.
- [ggcorrplot.args]{style="color:purple;"} argument provide additional (mostly aesthetic) arguments that will be passed to [ggcorrplot::ggcorrplot](http://www.sthda.com/english/wiki/ggcorrplot-visualization-of-a-correlation-matrix-using-ggplot2) function. The list should avoid any of the following arguments since they are already internally being used: [corr, method, p.mat, sig.level, ggtheme, colors, lab, pch, legend.title, digits]{style="color:purple;"}.
...
- to build a facet plot, the only argument needed is [grouping.var]{style="color:purple;"}.
- Behind *group_ggcorrmat()*, **patchwork** package is used to create the multiplot. [plotgrid.args]{style="color:purple;"} argument provides a list of additional arguments passed to [patchwork::wrap_plots](https://patchwork.data-imaginist.com/reference/wrap_plots.html), except for guides argument which is already separately specified earlier.
- Likewise, [annotation.args]{style="color:purple;"} argument is calling [plot annotation arguments](https://patchwork.data-imaginist.com/reference/plot_annotation.html) of patchwork package.
...
In the code chunk below, [read_csv()]{style="color:purple;"} of **readr** package is used to import *WHData-2018.csv* into R and save it into a tibble data frame object called wh.
...
Code chunk below shows a typical syntax used to plot a basic static parallel coordinates plot by using [ggparcoord()]{style="color:purple;"}.
...
Notice that only two argument namely [data]{style="color:purple;"} and [columns]{style="color:purple;"} is used. [Data]{style="color:purple;"} argument is used to map the data object (i.e. wh) and [columns]{style="color:purple;"} is used to select the columns for preparing the parallel coordinates plot.

### 15.4.2 Plotting a parallel coordinates with boxplot

The basic parallel coordinates failed to reveal any meaning understanding of the World Happiness measures. In this section, we will learn how to makeover the plot by using a collection of arguments provided by [ggparcoord()]{style="color:purple;"}.
...
- [groupColumn]{style="color:purple;"} argument is used to group the observations (i.e. parallel lines) by using a single variable (i.e. Region) and colour the parallel coordinates lines by region name.
- [scale]{style="color:purple;"} argument is used to scale the variables in the parallel coordinate plot by using [uniminmax]{style="color:purple;"} method. The method univariately scale each variable so the minimum of the variable is zero and the maximum is one.
- [alphaLines]{style="color:purple;"} argument is used to reduce the intensity of the line colour to 0.2. The permissible value range is between 0 to 1.
- [boxplot]{style="color:purple;"} argument is used to turn on the boxplot by using logical [TRUE]{style="color:purple;"}. The default is [FALSE]{style="color:purple;"}.
- [title]{style="color:purple;"} argument is used to provide the parallel coordinates plot a title.

### 15.4.3 Parallel coordinates with facet

Since [ggparcoord()]{style="color:purple;"} is developed by extending **ggplot2** package, we can combination use some of the ggplot2 function when plotting a parallel coordinates plot.

In the code chunk below, [facet_wrap()]{style="color:purple;"} of **ggplot2** is used to plot 10 small multiple parallel coordinates plots. Each plot represent one geographical region such as East Asia.
...
- To rotate x-axis text labels, we use [axis.text.x]{style="color:purple;"} as argument to [theme()]{style="color:purple;"} function. And we specify [element_text(angle = 30)]{style="color:purple;"} to rotate the x-axis text by an angle 30 degree.

### 15.4.5 Adjusting the rotated x-axis text label

Rotating x-axis text labels to 30 degrees makes the label overlap with the plot and we can avoid this by adjusting the text location using [hjust]{style="color:purple;"} argument to theme’s text element with [element_text()]{style="color:purple;"}. We use [axis.text.x]{style="color:purple;"} as we want to change the look of x-axis text.
...
The code chunk below plot an interactive parallel coordinates plot by using [parallelPlot()]{style="color:purple;"}.
...
In the code chunk below, [rotateTitle]{style="color:purple;"} argument is used to avoid overlapping axis labels.
...
We can change the default blue colour scheme by using [continousCS]{style="color:purple;"} argument as shown in the code chunl below.
...
In the code chunk below, [histoVisibility]{style="color:purple;"} argument is used to plot histogram along the axis of each variables.
...
This slide shows us how to install a R package which is not available in cran.
  ...
  [1. If this is the first time we are installing a package from github, install devtools package by using the code below or else we can skip this step.]{style="color:red;"}
