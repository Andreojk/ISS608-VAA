...
In the code chunk below, [odbcConnectAccess()]{style="color:purple;"} of **[RODBC](https://cran.r-project.org/web/packages/RODBC/index.html)** package is used used to import a database query table into R.

::: {.callout-note title="Another way instead of converting from 64 bit to 32 bit"}
I would prefer a fool-proof path instead of switching to 32-bit system. Below is another way by first Install 64-bit **"Microsoft Access Database Engine 2016 Redistributable"**.
:::
...
```{r}
mins <- sales_report %>% group_by(Product) %>% slice(which.min(Sales))
maxs <- sales_report %>% group_by(Product) %>% slice(which.max(Sales))
ends <- sales_report %>% group_by(Product) %>% filter(Month == max(Month))
```
...
## 31.5 Static Information Dashboard Design: gt and gtExtras
...
### 31.5.1 Plotting a simple bullet chart with gt and gtExtras
...
### 31.6.2 Adding statistics with gt
...
report %>%
  ...
  gt() %>%
  fmt_number(columns = vars(Average), decimals = 2)
```
...
In the code chunk below [highlight_points]{style="color:purple;"} argument is used to show the minimum and maximum values points and [label]{style="color:purple;"} argument is used to label first and last values.
...
In the code chunk below [statline]{style="color:purple;"} argument is used to show the mean line.
...
Instead adding reference line, bandline can be added by using the [bandline]{style="color:purple;"} argument.
