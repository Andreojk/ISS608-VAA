...
For Take-home Exercise 2, my group members ([Hendra](https://norhendra-isss608.netlify.app/) and [Jin Yao](https://isss608-blessmebellburve.netlify.app/)) and I have decided to do [Mini-Challenge 1](https://vast-challenge.github.io/2025/MC1.html). Through our discussion (and the help of a wheel randomizer), we split the challenge's questions amongst ourselves. Hendra will be doing question 2, Jin Yao will be doing question 1 and I will be doing question 3.
...
- I want to find out the direct influences by the 3 artist. I will visualise the entire visNetwork and count number of work-type influence edges each artists has.
...
- I want to find out how many of the songs, albums etc. by the 3 artist has been referenced to others. I will visualise the entire visNetwork and count number of influence type edges each artists has.

  1. "InterpolatesFrom"
  2. "DirectlySamples"
  3. "InStyleOf"
  4. "LyricalReferenceTo"
  5. "CoverOf"
...
- The below plot tries to see draw a interactive network using influence metrics "InterpolatesFrom","InStyleOf","LyricalReferenceTo","CoverOf","DirectlySamples". Max hop = 2.
...
if (nrow(ego_edges) == 0) {
  ...
  # ─── 9. Print the full centrality table ───────────────────────────────────
  print(centrality_tbl)
  
  # ─── 10. Bar‐chart of top 10 nodes by betweenness ──────────────────────────
  centrality_tbl %>%
    slice_head(n = 10) %>%
...
## Affliation works with Musical Groups 
...
- "MemberOf", "ComposerOf", "PerformerOf","LyricistOf", "RecordedBy","ProducerOf","DistributedBy" only.
...
# ─── 0. PARAMETERS ─────────────────────────────────────────────────────────────
# Time window for “emerging” Oceanus Folk releases:
window_start <- 2030
window_end   <- 2040

# How many “years after first release” to consider for early‐career metrics:
early_window_years <- 5

# Influence‐type edges to count:
...
kg_raw    <- fromJSON("data/MC1_graph.json")
nodes_tbl <- as_tibble(kg_raw$nodes) %>%
  mutate(idx = row_number())  # give each node a numeric index for easy joins

links_tbl <- as_tibble(kg_raw$links)

# Build a numeric “from/to” index for every edge row (for fast lookups)
id_map <- nodes_tbl %>% select(id, idx)
edges_idx <- links_tbl %>%
  left_join(id_map, by = c("source" = "id"))  %>% rename(from = idx) %>%
  left_join(id_map, by = c("target" = "id"))  %>% rename(to   = idx) %>%
...
# We’ll pull out every Song node with genre == "Oceanus Folk", parse its release/written year,
# and then find for each Person the earliest year they contributed.

# 2a. Keep only Song nodes that are “Oceanus Folk”
songs_oceanus <- nodes_tbl %>%
  filter(`Node Type` == "Song", genre == "Oceanus Folk") %>%
  # parse release_year & written_year (if present) → take whichever is non‐NA/earlier
  mutate(
  ...
  # drop any songs that have no valid year
  filter(!is.na(first_year)) %>%
  # now keep only those whose first_year is within [window_start, window_end]
  filter(first_year >= window_start, first_year <= window_end) %>%
  select(song_idx = idx, first_year)

# 2b. For each Person, find their “first Oceanus Folk year” by looking at edges
person_idxs <- nodes_tbl %>%
  filter(`Node Type` == "Person") %>%
  pull(idx)

# Get all ComposerOf / PerformerOf edges that point TO an Oceanus Folk song
# (so we know which people contributed to an Oceanus Folk song, and when)
...
# Join back to get that song’s first_year, then pick the earliest per person
person_first_oceanus <- person_song_edges %>%
  left_join(songs_oceanus, by = c("song_idx")) %>%
  group_by(person_idx) %>%
  summarise(
    first_oceanus_year = min(first_year, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  # Only keep those whose first oceanus year is in [window_start, window_end]
  filter(first_oceanus_year >= window_start,
         first_oceanus_year <= window_end)

# If nobody emerges in that window, we can stop early
if (nrow(person_first_oceanus) == 0) {
  stop("No emerging Oceanus Folk artists found between ", window_start, " and ", window_end, ".")
}

# ─── 3. COMPUTE EARLY‐CAREER “POPULARITY” (NOTABLE SONG COUNTS) ─────────────
# For each emerging artist, count how many of their Oceanus Folk songs within
# [first_oceanus_year, first_oceanus_year + early_window_years] are notable.

# 3a. Tag each Oceanus Folk song with its year and its “notable” flag
songs_oceanus_full <- nodes_tbl %>%
  ...
  mutate(
    ...
    is_notable     = if_else(!is.na(notoriety_date), TRUE, FALSE)
  ) %>%
  select(song_idx = idx, first_year_song, is_notable)

# 3b. For each emerging artist, list all their Oceanus Folk songs (with years & flags)
...
# 3c. Filter those “within early_window_years” of their first Oceanus Folk release
...
# 3d. Count notable songs per emerging artist in that early window
popularity_tbl <- artist_songs_early %>%
  filter(is_notable) %>%
  count(person_idx, name = "early_notable_songs")

# If an emerging artist has zero notable songs in the early window, give them a 0
...
# ─── 4. COMPUTE EARLY‐CAREER “INFLUENCE” (INFLUENCE‐EDGE COUNTS) ─────────────
# For each emerging artist, count how many influence‐type edges they are involved in,
# where that edge’s “timestamp” (we’ll approximate by “max of connected node’s year”) 
# falls within [first_oceanus_year, first_oceanus_year + early_window_years].

# 4a. First, let’s annotate each node with a “node_year” = earliest valid year (release/written/notoriety)
...
# 4b. Filter the master edge list to only those influence‐types and where one endpoint is an emerging artist
artist_infl_edges <- edges_idx %>%
  ...
  # annotate each edge with “edge_year” = max( node_year[from], node_year[to] )
...
# 4c. Join to get each artist’s first_oceanus_year, then keep only if edge_year is within the early window
...
# 4d. Attach zeros for artists with no early influence edges
...
emerging_metrics <- person_first_oceanus %>%
  ...
  # Join in the actual Person name
  left_join(
    nodes_tbl %>% select(person_idx = idx, name),
    by = "person_idx"
  ) %>%
  mutate(
    total_score = early_notable_songs + early_influence_edges
    # You could weight these differently if you like: 
    # total_score = 0.6 * early_notable_songs + 0.4 * early_influence_edges, etc.
  ) %>%
...
# 5a. If you just want the top 3 predictions:
top3_emerging <- emerging_metrics %>% slice_head(n = 3)

# ─── 6. OUTPUT RESULTS ────────────────────────────────────────────────────────
message("All emerging Oceanus Folk artists (", window_start, "–", window_end, 
        ") ranked by early‐career score:\n")
print(emerging_metrics)

message("\nTop 3 predictions for next Oceanus Folk stars:\n")
print(top3_emerging)
...
- I will do a global count of every Oceanus Folk song marked "notable == True" to have an overview without any restrictions. I will plot the total songs released beside it as comparison.
...
### Names of artist's first release songs from 2025 - 2035

Based on the time period, it seems that from 2035-2040, there were no notable songs released by Oceanus Folk people. This makes the range to predict much earlier especially from 2021 onwards where alot of songs were released with notable charting.
...
# 2. Extract Oceanus Folk songs and compute their “song_year”
oceanus_songs <- nodes_tbl %>%
  filter(`Node Type` == "Song", genre == "Oceanus Folk") %>%
  mutate(
    # parse year from release_date or written_date
    release_year = suppressWarnings(as.integer(str_extract(release_date, "\\d{4}"))),
    written_year = suppressWarnings(as.integer(str_extract(written_date, "\\d{4}"))),
    song_year    = pmin(release_year, written_year, na.rm = TRUE)
  ) %>%
...
# 4. Find “ComposerOf” or “PerformerOf” edges linking persons to those Oceanus Folk songs
person_song_edges <- edges_idx %>%
  filter(
    `Edge Type` %in% c("ComposerOf", "PerformerOf"),
    from %in% person_idxs,
    to   %in% oceanus_songs$song_idx
  ) %>%
  select(person_idx = from, song_idx = to)

# 5. Join to get each linked song’s year, then find each person’s earliest song_year
person_first_oceanus <- person_song_edges %>%
  left_join(oceanus_songs, by = "song_idx") %>%
  group_by(person_idx) %>%
  summarize(
    first_oceanus_year = min(song_year, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  # 6. Keep only those whose first release is between 2025 and 2040 (inclusive)
  filter(first_oceanus_year >= 2025, first_oceanus_year <= 2035)

# 7. Attach each person’s name
result_tbl <- person_first_oceanus %>%
  left_join(
    nodes_tbl %>% select(person_idx = idx, PersonName = name),
    by = "person_idx"
  ) %>%
  select(PersonName, first_oceanus_year) %>%
  arrange(first_oceanus_year, PersonName)

# 8. Print the result
if (nrow(result_tbl) == 0) {
  message("No Oceanus Folk persons found with first‐release between 2025 and 2040.")
} else {
  print(result_tbl)
}
...
### Notable Oceanus Folk songs based on debut of Artist from 2025 to 2035
...
### Pie-Chart of recent Oceanus Folk people

- It's worth to note that Sailor Shift has more notable songs in the pie-chart plot as compared to the previous plot. However, 

The reason Sailor Shift shows up with only one “notable” Oceanus Folk song in that 2025–2035 plot is due to the combination of two filters:

Emerging‐artist filter (first release between 2025–2035):

We only include persons whose first Oceanus Folk song falls in the window 2025–2035. If Sailor Shift’s actual first Oceanus Folk release in our data happens, say, in 2028, then she is included—but if her earliest track was before 2025 (e.g. 2023 or 2024), she would be dropped entirely.

“Notable” + year filter (only count songs whose notoriety_date exists and whose song_year is 2025–2035):

Even once Sailor Shift is on the list (because her first‐year = 2028, for example), we then gather all her Oceanus Folk songs from 2025–2035 that have a non‐NA notoriety_date. If only one of her Oceanus Folk songs is marked “notable” within that 2025–2035 window, then the plot will show a total of one for her.

In short, although Sailor Shift may have released multiple Oceanus Folk tracks, only one of those tracks both (a) lies between 2025–2035 in its “song_year,” and (b) has a non‐NA notoriety_date. All other Oceanus Folk songs either:

Have a song_year outside 2025–2035, or

Don’t have a notoriety_date (so they’re not counted as “notable”),



::: {.callout-note title="Notable Oceanus Folk songs"}
- Both these plots now picture the top 3 "popular" artist based on using notable Oceanus Folk songs as proxy.
Despite Sailor Shift having more songs, only 3 of it are notable (Only 1 of it has a notoriety_date) as compared to the other songs by similar debutant artist.

- With this, **Orla Seabloom, Beatrice Albright and Daniel O'Connell** are the most promising upcoming Oceanus Folk artists.
:::

## Influences

- I will plot the influence diagram without splitting the work and influence related edges. This will show a good overview. 
...
# Print the full betweenness table
print(betw_tbl)
